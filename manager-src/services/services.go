...
func StartMariaDB(rootDir string) error {
	...
	mariaDbDir := filepath.Join(rootDir, "mariadb")
	dataDir := filepath.Join(mariaDbDir, "data")

	// 3. Auto-initialize data directory on first run
...
func StopMariaDB(rootDir string) error {
	...
	exe := filepath.Join(rootDir, "mariadb", "bin", "mysqladmin.exe")
...
func fixPHPIni(rootDir string) error {
	phpDir := filepath.Join(rootDir, "php")
...
func StartPHPMyAdmin(rootDir string) error {
	...
	if !isPortOpen("8080") {
		...
		phpExe := filepath.Join(rootDir, "php", "php.exe")
		pmaDir := filepath.Join(rootDir, "phpmyadmin")
...
func StartMailpit(rootDir string) error {
	...
	if !isPortOpen("8025") {
		...
		exe := filepath.Join(rootDir, "mailpit", "mailpit.exe")
...
func ViewPHPIni(rootDir string) error {
	...
	iniPath := filepath.Join(rootDir, "php", "php.ini")
